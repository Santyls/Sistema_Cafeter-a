...
export default function ClienteMesero(props) {
  ...
  const handleRecovery = () => {
    ...
    fetch('http://192.168.1.7:5001/api/auth/recuperar-password', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const updateTableStatusOnBackend = (tableId, estado) => {
    if (!token) return;
    fetch(`http://192.168.1.7:5001/api/mesas/${tableId}/estado`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
  ...
  const handleConfirmOrder = () => {
    ...
    fetch('http://192.168.1.7:5001/api/pedidos', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
  ...
  const handleConfirmPayment = () => {
    setIsPaying(true);
    fetch(`http://192.168.1.7:5001/api/mesas/${billingTableId}/estado`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
  ...
  const handleUpdateProfile = () => {
    ...
    setIsLoggingIn(true);
    fetch(`http://192.168.1.7:5001/api/usuarios/${currentUser?.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
                                      ...
                                      Alert.alert(
                                        ...
                                        [
                                          { text: 'Cancelar', style: 'cancel' },
                                          {
                                            text: 'Sí, notificar',
                                            onPress: () => {
                                              fetch(`http://192.168.1.7:5001/api/mesas/${table.id}/estado`, {
                                                method: 'PATCH',
                                                headers: {
                                                  'Content-Type': 'application/json',
                                                  'Authorization': `Bearer ${token}`
                  ...
                  <ScrollView style={styles.contentContainer} showsVerticalScrollIndicator={false}>
                    ...
                    {trackingOrder.status !== 'entregado' && trackingOrder.status !== 'cancelado' && (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          fetch(`http://192.168.1.7:5001/api/pedidos/${selectedTrackingOrderId}/estado`, {
                            method: 'PATCH',
                            headers: {
                              'Content-Type': 'application/json',
                              'Authorization': `Bearer ${token}`
              ...
              <View style={styles.formCard}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    fetch(`http://192.168.1.7:5001/api/reservaciones`, {
                      method: 'POST',
                      headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`
                                  ...
                                  Alert.alert(
                                    ...
                                    [
                                      { text: 'Cancelar', style: 'cancel' },
                                      {
                                        text: 'Sí, notificar',
                                        onPress: () => {
                                          fetch(`http://192.168.1.7:5001/api/mesas/${selectedTableForDrawer.id}/estado`, {
                                            method: 'PATCH',
                                            headers: {
                                              'Content-Type': 'application/json',
                                              'Authorization': `Bearer ${token}`
