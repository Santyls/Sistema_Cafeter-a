...
export default function CorteCaja({ cambiarPantalla, toggleSidebar, token, usuarioLogueado, idCajaActiva }) {
  ...
  React.useEffect(() => {
    ...
    // Load Caja Info
    const fetchCaja = fetch(`http://192.168.1.7:5001/api/caja/${idCajaActiva}`, {
      headers: { 'Authorization': `Bearer ${token}` }
    }).then(res => res.json());

    // Load Tickets
    const fetchTickets = fetch(`http://192.168.1.7:5001/api/tickets?id_caja=${idCajaActiva}&with_pagos=true`, {
      headers: { 'Authorization': `Bearer ${token}` }
    }).then(res => res.json());

    // Load Gastos
    const fetchGastos = fetch(`http://192.168.1.7:5001/api/gastos`, {
      headers: { 'Authorization': `Bearer ${token}` }
    }).then(res => res.json());
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.body} style={{ backgroundColor: Colors.background }}>
        ...
        <PrimaryButton
          ...
          onPress={() => {
            if (idCajaActiva) {
              fetch(`http://192.168.1.7:5001/api/caja/${idCajaActiva}/cerrar`, {
                method: 'PATCH',
                headers: { 'Authorization': `Bearer ${token}` }
              })
