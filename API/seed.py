...
try:
    ...
    session.add_all([admin, mesero, cocinero, cajero])
    ...
    session.add_all([cat_bebidas, cat_frias, cat_postres])
    session.flush()
    ...
    session.add_all([cafe_grano, leche])
    session.flush()
    ...
    session.add_all([americano, capuchino])
    session.flush()
